...
The dWeb does away with blockchains and the Web3 model, replacing them with Distributed Machines that can be executed and queries via dWeb's distributed web-based architecture. Distributed Machines have many notable differences when compared to blockchains:

- They are built around a single smart contract.
- The users of a Distributed Machine are also its participants, processing contract code locally on their own machine and storing the results of those executions locally in their own UniChain, or “operator log.”
...
- Validators and other third parties can replay operator logs and output logs in an attempt to produce the same index log. If a validator produces an index log that matches the Distributed Machine’s published index log, then the data is validated and considered organic. This validation is possible because executors deterministically apply operations from participant oplogs to the index log, linearly.
